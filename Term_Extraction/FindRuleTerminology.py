...
def getTermRule(paper,userdict=data_dir+"/Data/user.txt",stop_flag=None,n_gram=[2,6],max_step=10):
    '''
    统计术语词性组成规则
    Parameter:
        - paper: 每个领域的文章
    ...
    for code in paper:
        ...
        for tmp in code_rule:
            if tmp not in all_rule:
                all_rule[tmp]={}
            for l_w in  code_rule[tmp]:
                if l_w not in all_rule[tmp]:
                    all_rule[tmp][l_w]=0
                all_rule[tmp][l_w]+=code_rule[tmp][l_w]

        file_dict={i:open(data_dir+"/Data_tmp/rule_"+code+"_"+str(i)+'.txt','w',encoding='utf8')   for i in range(n_gram[0],n_gram[1]+1)}
    ...
    temp=sorted(all_rule.items(),key=lambda x:x[1]['all'],reverse=True)
    for t in temp:
        size=len(t[0].split("_"))
